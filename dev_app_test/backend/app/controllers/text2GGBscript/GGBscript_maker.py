...
def rectangular(part_object: dict[str,str,dict]):
    ...
    # 図形の名前がABED-EFGH型の正規表現に従うかを検査
    if not re.match(r"^[A-Z]{4}-[A-Z]{4}$", object_name, re.IGNORECASE):
...
def Cube(part_object: dict[str,str,dict]):
    ...
    ggbscript = f"{object_name[0]} = (0,0,0)\n"
    ggbscript += f"{object_name[1]} = ({length},0,0)\n"
    ggbscript += f"{object_name[2]} = ({length},{length},0)\n"
    ggbscript += f"{object_name[3]} = (0,{length},0)\n"
    ggbscript += f"{object_name[5]} = (0,0,{length})\n"
    ggbscript += f"{object_name[6]} = ({length},0,{length})\n"
    ggbscript += f"{object_name[7]} = ({length},{length},{length})\n"
    ggbscript += f"{object_name[8]} = (0,{length},{length})\n"
...
def tetrahedron(part_object: dict[str,str,dict]):
    ...
    # 辺の長さ変数を初期化
    sides = [0] * 6
    # パラメータリストから辺長を取り出して変数に格納
    for num, side in enumerate(parameters['side_name_and_length']):
        sides[num] = float(side["side_length"])
    for out_num in range(num+1, 6):
        avg = sum(sides[:out_num]) / out_num
        sides[out_num] = avg
        
    a_val, b_val, c_val, d_val, e_val, f_val = sides
    ...
    # 図形の名前がABED-EFGH型の正規表現に従うかを検査
    if not re.match(r"^[A-Z]{4}$", object_name, re.IGNORECASE):
        object_name = "PABC"
    # GGBスクリプトを生成
    ggbscript = f"{object_name[0]} = {str(P)}\n"
    ggbscript += f"{object_name[1]} = {str(A)}\n"
    ggbscript += f"{object_name[2]} = {str(B)}\n"
    ggbscript += f"{object_name[3]} = {str(C)}\n"
    ggbscript += f"{object_name[0]}{object_name[1]} = Segment({object_name[0]},{object_name[1]})\n"
    ggbscript += f"{object_name[0]}{object_name[2]} = Segment({object_name[0]},{object_name[2]})\n"
    ggbscript += f"{object_name[0]}{object_name[3]} = Segment({object_name[0]},{object_name[3]})\n"
    ggbscript += f"{object_name[1]}{object_name[2]} = Segment({object_name[1]},{object_name[2]})\n"
    ggbscript += f"{object_name[1]}{object_name[3]} = Segment({object_name[1]},{object_name[3]})\n"
    ggbscript += f"{object_name[2]}{object_name[3]} = Segment({object_name[2]},{object_name[3]})\n"
    return ggbscript
